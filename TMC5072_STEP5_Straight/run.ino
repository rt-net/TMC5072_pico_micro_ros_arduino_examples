...
void TMC5072Straignt(t_TMC5072_position tmc5072_position)
{
  ...
  while (
    (int)((abs((int)TMC5072Read_no_status(TMC5072_XACTUAL1)) + abs((int)TMC5072Read_no_status(TMC5072_XACTUAL2))) / 2.0 * TMC5072_PULSE) <
    ((int)(tmc5072_position.len) - 1)) {
    continue;
  };
...
void decelerate_VELOCITY(int len, int tar_speed)
{
  ...
  while (
    len - (abs((int)TMC5072Read_no_status(TMC5072_XACTUAL1)) +
           abs((int)TMC5072Read_no_status(TMC5072_XACTUAL2))) /
            2.0 * TMC5072_PULSE >
    ((((int)(TMC5072Read_no_status(TMC5072_VACTUAL1) * TMC5072_VELOCITY * TMC5072Read_no_status(TMC5072_VACTUAL1) * TMC5072_VELOCITY)) -
      (MIN_SPEED * MIN_SPEED)) /
     (2.0 * 1000.0 * g_accel))) {
    continue;
  }
